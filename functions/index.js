...
- "tipo" (despesa/receita) é determinado SOMENTE pela natureza da operação — a coluna Crédito/Débito, o sinal (+/-) ou marcador "C"/"D" impresso no documento. O histórico/descrição da linha NUNCA é usado para decidir "tipo" — ele só serve de base para escolher a "categoria". Armadilha comum (achado 13/07/2026, extrato Bradesco real): uma linha "PAGTO ELETRON COBRANCA BANCO INTER - DEPOSITO POR BOLETO" com valor na coluna Débito é uma DESPESA (a usuária pagando um boleto por depósito) — a palavra "depósito" ali descreve o MÉTODO do pagamento, não dinheiro entrando na conta; não deixe o texto te enganar, confie só no indicador estrutural. Se a linha genuinamente não tiver nenhum indicador estrutural de crédito/débito (documento sem essa distinção visível, ex. texto colado sem coluna nem sinal), marque "tipoIncerto": true e ainda assim preencha "tipo" com seu melhor palpite — nunca decida com confiança só pelo texto quando não há indicador.
...
- Dentro de uma FATURA (o documento é a própria fatura, não um extrato de conta corrente), uma linha de valor negativo ou marcado como crédito representando o pagamento da fatura anterior sendo aplicado (ex: "Pagamentos Válidos", "Pagamento Recebido", "Crédito", "Estorno de pagamento") NÃO é receita nem despesa — é só informativo (mostra o saldo anterior sendo quitado). Não inclua em "itens" nem em "pagamentosFatura" (essa regra é só para extrato de conta corrente); ignore a linha, igual a um saldo/resumo. Diferente é um estorno de UMA COMPRA específica (ex: "Estorno Amazon") — esse sim é receita normal, categoria "Restituição / Estorno".
